# Example:
#   docker run -d -v $(readlink -f znc-state):/znc-state millerlogic/znc-container
...
# Get source.
RUN wget -O/tmp/znc-1.4.tar.gz http://znc.in/releases/znc-1.4.tar.gz
RUN cd /tmp && tar xzf znc-1.4.tar.gz

# Build.
RUN cd /tmp/znc-1.4/ && ./configure --enable-perl --enable-python --enable-tcl \
    --enable-openssl --enable-ipv6 | grep -v 'yes$'
RUN cd /tmp/znc-1.4/ && make && make install
